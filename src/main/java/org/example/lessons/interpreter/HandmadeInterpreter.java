package org.example.lessons.interpreter;
...
class Demo {
  ...
  static Element parse(List<Token> tokens) {
    ...
    for (int i = 0; i < tokens.size(); ++i) {
      ...
      switch (token.type) {
        case INTEGER -> {
          IntegerElement integerElement = new IntegerElement(Integer.parseInt(token.text));
          if (!haveLHS) {
        ...
        case LPAREN -> {
          ...
          List<Token> subexpression = tokens.stream().skip(i + 1).limit(j - i - 1).toList();
